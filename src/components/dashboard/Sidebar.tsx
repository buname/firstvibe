    ...
    >
      <div className="shrink-0 flex items-center px-1.5 pt-2 pb-1">
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-neutral-400 transition-colors hover:bg-white/[0.06] hover:text-neutral-200"
        >
        ...
        {SIDEBAR_GROUPS.map((group, groupIndex) => {
          ...
          return (
            <div key={groupIndex}>
              {groupIndex > 0 && (
                <div
                  className={`mx-1.5 h-px bg-white/[0.08] ${collapsed ? "my-1.5" : "my-2"}`}
                  aria-hidden
                />
              )}
